...
预算：350RMB/人Total:6人=2100RMB`
    ...
    const appState = {
        ...
        get allRecognized() { return this.recognized.waybill && this.recognized.invoice && this.recognized.attachment; }
    };
    ...
    imageControls.addEventListener('click', (event) => {
        if (event.target.tagName === 'BUTTON') {
            const zoomAction = event.target.dataset.zoom;
            const navAction = event.target.dataset.nav;
    ...
    function updateDisplay(action) {
        ...
        switch (data.format) {
            ...
            case 'text':
                ...
                break;
            default:
                contentHtml = `<pre>${JSON.stringify(data.results, null, 2)}</pre>`;
        }
    ...
    function handleZoom(direction) {
        ...
        if (direction === 'in') {
            level = Math.min(level + ZOOM_STEP, MAX_ZOOM);
        } else if (direction === 'out') {
            level = Math.max(level - ZOOM_STEP, MIN_ZOOM);
        }
    ...
    function renderXml(xmlString) {
        ...
        function parseNode(node) {
            ...
            if (children.length > 0) {
                ...
                children.forEach(child => parseNode(child));
                html += '</ul></li>';
            } else {
